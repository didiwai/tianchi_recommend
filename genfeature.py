...
def genFinalFeatureToFile(end_date, frname, fwname):
	...
	time_to_subtract = [0,2,4,6,13,30]
	...
	with open(fwname, "w") as fw:
		...
		for node in useritempair:
			...
			feat = list()
			#生成用户特征
			query = ("SELECT count(id),count(distinct itemid),count(distinct pytime) FROM trainuser_all "
		         "WHERE userid=%s and behavior=%s and pytime between %s and %s")
			query_1 = ("SELECT count(distinct pytime) FROM trainuser_all "
		         "WHERE userid=%s and pytime between %s and %s")
			query_2 = ("SELECT min(pytime),max(pytime) FROM trainuser_all "
		         "WHERE userid=%s and behavior=%s")
			useraction = dict()
			...
			query_1 = ("SELECT count(id) FROM trainuser_all "
			...
			#生成用户-品牌特征
			query = ("SELECT count(id),count(distinct pytime) FROM trainuser_all "
			...
			query_1 = ("SELECT min(pytime),max(pytime) FROM trainuser_all "
		         "WHERE userid=%s and itemid=%s and behavior=%s")
			data = (user, item, 1)
			cursor.execute(query_1, data)
			rs = cursor.fetchall()
			user_first_click_item = rs[0][0];user_last_click_item = rs[0][1]
			if user_first_click_item == None or user_last_click_item == None:
				feat.append(0)
			else:
				temp = (user_last_click_item - user_first_click_item).days
				feat.append(temp)
			data = (user, item, 4)
			cursor.execute(query_1, data)
			rs = cursor.fetchall()
			user_first_buy_item = rs[0][0];user_last_buy_item = rs[0][1]
			if user_first_buy_item == None or user_last_buy_item == None:
				feat.append(0)
			else:
				temp = (user_last_buy_item - user_first_buy_item).days
				feat.append(temp)
...
if __name__ == "__main__":
	...
	genFinalFeatureToFile(end_date, "offline_test_data.csv", "testdata_feature_offline.csv")
